...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  Client-side routing is when users download all the data necessary for an app to function when they initially load the page.
  This leads to a slower initial load time but smooth running useability from then on.
